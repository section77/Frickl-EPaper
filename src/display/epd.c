...
void epd_dspImage(char* img[VRES][HRES >> 3])
{
    sendCommand(0x13);
    for (uint16_t y = 0; y < VRES; y++) {
        for (uint16_t x = 0; x < (HRES >> 3);x++) {
            sendData((uint8_t)image1[y][x-1]);
        }
    }
...
void epd_init()
{
    ...
    sendCommand(6); // Booster Soft Start
    sendData(0x17);
    sendData(0x17);
    sendData(0x17);
    ...
    sendCommand(0); // Panel Setting
    sendData(0x0f);
    sendData(0x0d); //?

    sendCommand(0x61);  // Resolution Setting (TRES)
    sendData(HRES);
    sendData(VRES >> 8);
    sendData(VRES);
